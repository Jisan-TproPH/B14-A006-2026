...
export default function LibraryCard({ data }: { data: Workout }) {
    return (
        <Link href={`/workouts/${data.id}`}
            ...
            <div
                className="bg-[#15171D]  w-full rounded-2xl flex flex-col overflow-hidden"
            >
                <div className="">
                    <Image className="w-full h-auto max-h-64 object-cover" src={data.image} alt={data.name} width={400} height={200} />
                </div>
